...
export default class ApiResponse {
  ...
  error(payload: ResponseJson) {
    return this._response
      .status(payload.statusCode ? payload.statusCode : 500)
      .json({
        error: true,
  ...
  errorObj(payload: unknown) {
    return this._response.status(500).json({
      error: true,
      message:
        payload instanceof Error ? payload.message : "something went wrong",
      statusCode: 500,
      errors:
        payload instanceof Error && "cause" in payload ? payload.cause : "",
    });
  }
}
